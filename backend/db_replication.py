...
"""
    # Write config to replica via SSH
    cmd = _ssh_cmd(replica_host)
    cmd.append(f"sudo tee /etc/mysql/mysql.conf.d/moodle-dr-replica.cnf << 'EOF'\n{mycnf_content}\nEOF")
    r = run_cmd(cmd, timeout=20)
    db_db.log_audit("configure_replica", result="ok" if r["ok"] else "error",
                    details={"host": replica_host})
    return {"ok": r["ok"], "message": "Replica configuration deployed", "error": r["stderr"] if not r["ok"] else None}
